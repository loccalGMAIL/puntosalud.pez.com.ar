...
@section('title', 'Nuevo Tipo de Movimiento - ' . config('app.name'))
@section('mobileTitle', 'Nuevo Tipo')
...
<div class="flex h-full flex-1 flex-col gap-6 p-4">
    ...
    <div class="flex flex-col gap-4">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">
                Crea un nuevo tipo o categoría de movimiento de caja
            </p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
        <form action="{{ route('movement-types.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description"
                                  ...
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-gray-700 dark:text-white @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category"
                            ...
                            <option value="main_type" {{ old('category') == 'main_type' ? 'selected' : '' }}>Tipo Principal</option>
                            <option value="expense_detail" {{ old('category') == 'expense_detail' ? 'selected' : '' }}>Detalle de Egreso</option>
                            <option value="income_detail" {{ old('category') == 'income_detail' ? 'selected' : '' }}>Detalle de Ingreso</option>
                            <option value="withdrawal_detail" {{ old('category') == 'withdrawal_detail' ? 'selected' : '' }}>Detalle de Retiro</option>
                        </select>
                        @error('category')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="parent_type_id"
                            ...
                            @foreach($parentTypes as $parentType)
                                <option value="{{ $parentType->id }}" {{ old('parent_type_id') == $parentType->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="affects_balance"
                            ...
                            <option value="1" {{ old('affects_balance') == '1' ? 'selected' : '' }}>+1 (Ingreso)</option>
                            <option value="0" {{ old('affects_balance') == '0' ? 'selected' : '' }}>0 (Neutral)</option>
                            <option value="-1" {{ old('affects_balance') == '-1' ? 'selected' : '' }}>-1 (Egreso)</option>
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="color"
                            ...
                            <option value="red" {{ old('color') == 'red' ? 'selected' : '' }}>Rojo</option>
                            <option value="yellow" {{ old('color') == 'yellow' ? 'selected' : '' }}>Amarillo</option>
                            <option value="purple" {{ old('color') == 'purple' ? 'selected' : '' }}>Púrpura</option>
                            <option value="orange" {{ old('color') == 'orange' ? 'selected' : '' }}>Naranja</option>
