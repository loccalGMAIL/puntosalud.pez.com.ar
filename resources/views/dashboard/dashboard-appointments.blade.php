                        ...
                        <div class="text-right flex flex-col items-end gap-2">
                            <div class="flex items-center gap-3">
                                <div>
                                 ...
                                 <!-- Botones de acción -->
                                  <div class="flex gap-1" x-data="appointmentActions({{ $consulta['id'] }}, {{ $consulta['monto'] ?? 0 }}, {{ !empty($consulta['hasPhone']) ? 'true' : 'false' }})">
                                       @if($consulta['isPaid'])
                                        ...
                                        <button type="button"
                                                @click="shareReceipt({{ (int) $consulta['paymentId'] }})"
                                                class="p-1.5 text-xs rounded-md border transition-colors duration-200 bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-700 dark:bg-slate-900/20 dark:hover:bg-slate-900/30 dark:border-slate-800 dark:text-slate-300"
                                                title="Compartir recibo">
                                            <i class="fa-solid fa-share-nodes text-[0.85rem] leading-none"></i>
                                        </button>
                                    @endif
                                    ...
                                    @endif
                                     ...
                                     @endif
                                       ...
                                       @if($consulta['status'] !== 'attended')
                                           <button @click="sendWhatsAppReminder()" :disabled="loading || !hasPhone"
                                                   class="p-1.5 text-xs rounded-md border transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed bg-emerald-50 hover:bg-emerald-100 border-emerald-200 text-emerald-700 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/30 dark:border-emerald-800 dark:text-emerald-300"
                                                   :title="hasPhone ? 'Enviar recordatorio por WhatsApp' : 'El paciente no tiene teléfono cargado'">
                                               <i class="fa-brands fa-whatsapp text-[0.85rem] leading-none"></i>
                                           </button>
                                       @endif
                                  </div>
                            </div>
...
// Abrir modal de cobro (multi-forma) reutilizable
function openMultiPaymentModal(appointmentId, estimatedAmount = 0, hasPhone = true) {
    ...
    modal.open(estimatedAmount, async (paymentData) => {
        ...
        try {
            if (result.payment_id) {
                ...
                if (action === 'whatsapp') {
                    await window.sharePaymentReceiptByWhatsApp(result.payment_id);
                }
            }
        } catch (error) {
...
function appointmentActions(appointmentId, estimatedAmount = 0, hasPhone = true) {
    return {
        loading: false,
        hasPhone: !!hasPhone,

        async shareReceipt(paymentId) {
            try {
                const action = await window.askReceiptAction({
                    paymentId,
                    kind: 'patient_payment',
                    hasPatientPhone: !!this.hasPhone
                });

                if (action === 'print') {
                    window.open(`/payments/${paymentId}/print-receipt?print=1`, '_blank');
                }

                if (action === 'whatsapp') {
                    await window.sharePaymentReceiptByWhatsApp(paymentId);
                }
            } catch (error) {
        ...
        async sendWhatsAppReminder() {
            if (!this.hasPhone) {
                await DashboardAPI.showNotification('El paciente no tiene un número de teléfono registrado.', 'error');
                return;
            }
            ...
            try {
                await DashboardAPI.makeRequest(`/dashboard/appointments/${appointmentId}/send-whatsapp-reminder`, {
                    method: 'POST'
                });

                await DashboardAPI.showNotification('Recordatorio enviado por WhatsApp', 'success');
            } catch (error) {
        ...
        markCompletedAndPaid() {
            openMultiPaymentModal(appointmentId, estimatedAmount, this.hasPhone);
        },
