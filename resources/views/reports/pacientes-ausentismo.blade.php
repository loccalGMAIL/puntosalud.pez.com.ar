...
@section('title', 'Ausentismo de Pacientes - ' . config('app.name'))
@section('mobileTitle', 'Ausentismo')

@section('content')
<div class="p-6" x-data="ausentismoForm()">
    ...
    <div class="mb-6">
        <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-2">
            ...
            <span>Reportes</span>
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
            <span>Pacientes – Ausentismo</span>
        </nav>
        <div class="flex items-start justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Pacientes – Ausentismo</h1>
                <p class="text-gray-600 dark:text-gray-400">Tasa de inasistencia por profesional</p>
            </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-yellow-200 dark:border-yellow-700 p-4">
            <p class="text-xs font-medium text-yellow-600 dark:text-yellow-400">Total Ausentes</p>
            <p class="text-2xl font-bold text-yellow-700 dark:text-yellow-400">{{ number_format($globalAbsent) }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">Tasa Global</p>
            <p class="text-2xl font-bold {{ $globalRate > 20 ? 'text-red-600 dark:text-red-400' : ($globalRate > 10 ? 'text-yellow-600 dark:text-yellow-400' : 'text-emerald-600 dark:text-emerald-400') }}">{{ $globalRate }}%</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">Profesionales</p>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-600 text-xs text-gray-500 dark:text-gray-400 uppercase">
                        ...
                        <th class="text-center py-3 px-4 font-medium">Tasa Ausencia</th>
                        <th class="text-left py-3 px-4 font-medium hidden lg:table-cell" style="min-width:150px">Distribución</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="py-3 px-4 text-sm font-medium text-gray-900 dark:text-white">{{ $s['full_name'] }}</td>
                        <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-400 hidden md:table-cell">{{ $s['specialty'] }}</td>
                        ...
                        <td class="py-3 px-4 text-sm text-center">
                            <span class="font-bold {{ $s['absence_rate'] > 20 ? 'text-red-600 dark:text-red-400' : ($s['absence_rate'] > 10 ? 'text-yellow-600 dark:text-yellow-400' : 'text-emerald-600 dark:text-emerald-400') }}">
                                {{ $s['absence_rate'] }}%
                            </span>
                        </td>
                        <td class="py-3 px-4 hidden lg:table-cell">
                            <div class="flex h-2 rounded-full overflow-hidden">
                                <div class="bg-emerald-500" style="width: {{ round($attendedPct, 1) }}%"></div>
                                <div class="bg-yellow-400" style="width: {{ round($absentPct, 1) }}%"></div>
                            </div>
                        </td>
                    </tr>
                    @endforeach
    ...
    <p class="mt-3 text-xs text-gray-400 dark:text-gray-500">La tasa de ausencia se calcula sobre turnos completados (atendidos + ausentes). Tasa > 20% = alto; 10-20% = moderado; &lt; 10% = bajo.</p>
</div>

<script>
function ausentismoForm() {
    return {
        loading: false,
        filters: {
            date_from: '{{ $dateFrom }}',
        ...
        generateReport() {
            ...
            window.location.href = '{{ route('reports.pacientes.ausentismo') }}?' + params.toString();
        },
        printReport() {
            const params = new URLSearchParams();
            Object.entries(this.filters).forEach(([k, v]) => { if (v) params.set(k, v); });
            params.set('print', '1');
            window.open('{{ route('reports.pacientes.ausentismo.print') }}?' + params.toString(), '_blank');
        },
    }
}
</script>
