...
@section('title', 'Ingresos por Profesional - ' . config('app.name'))
@section('mobileTitle', 'Ingresos por Prof.')

@section('content')
<div class="p-6" x-data="profesionalesIngresosForm()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-2">
                ...
                <span>Reportes</span>
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
                <span>Profesionales – Ingresos</span>
            </nav>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Profesionales – Ingresos</h1>
            <p class="text-gray-600 dark:text-gray-400">Facturación por profesional en el período seleccionado</p>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
            ...
            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">${{ number_format($grandTotal, 2) }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Turnos Atendidos</p>
            <p class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{{ number_format($grandCount) }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Promedio por Turno</p>
            <p class="text-2xl font-bold text-gray-900 dark:text-white">${{ $grandCount > 0 ? number_format($grandTotal / $grandCount, 2) : '0.00' }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="p-5 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Detalle por Profesional ({{ $professionals->count() }})</h2>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-600 text-xs text-gray-500 dark:text-gray-400 uppercase">
                        ...
                        <th class="text-right py-3 px-4 font-medium">Turnos</th>
                        ...
                        <th class="text-left py-3 px-4 font-medium hidden lg:table-cell" style="width:200px">% del Total</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @foreach($professionals as $p)
                    @php $pct = $grandTotal > 0 ? ($p['total_amount'] / $grandTotal * 100) : 0; @endphp
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        ...
                        <td class="py-3 px-4 text-sm text-right text-gray-700 dark:text-gray-300">{{ $p['appointments'] }}</td>
                        <td class="py-3 px-4 text-sm text-right font-bold text-blue-700 dark:text-blue-400">${{ number_format($p['total_amount'], 2) }}</td>
                        <td class="py-3 px-4 text-sm text-right text-gray-600 dark:text-gray-400 hidden md:table-cell">${{ number_format($p['avg_amount'], 2) }}</td>
                ...
                <tfoot>
                    <tr class="border-t-2 border-gray-300 dark:border-gray-500 bg-gray-50 dark:bg-gray-700">
                        ...
                        <td class="py-3 px-4 text-sm text-right font-bold text-gray-900 dark:text-white">{{ $grandCount }}</td>
                        <td class="py-3 px-4 text-sm text-right font-bold text-blue-700 dark:text-blue-400">${{ number_format($grandTotal, 2) }}</td>
                        <td class="hidden md:table-cell"></td>
...
<script>
function profesionalesIngresosForm() {
    return {
        loading: false,
        filters: {
            date_from: '{{ $dateFrom }}',
        ...
        generateReport() {
            ...
            window.location.href = '{{ route('reports.profesionales.ingresos') }}?' + params.toString();
        },
        printReport() {
            const params = new URLSearchParams();
            Object.entries(this.filters).forEach(([k, v]) => { if (v) params.set(k, v); });
            params.set('print', '1');
            window.open('{{ route('reports.profesionales.ingresos.print') }}?' + params.toString(), '_blank');
        },
    }
}
</script>
