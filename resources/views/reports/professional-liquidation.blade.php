...
@section('content')
    <div class="container mx-auto px-4 py-6 max-w-4xl">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">Dr.
                        {{ $liquidationData['professional']->full_name }}</p>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        {{ $liquidationData['professional']->specialty->name }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">Fecha</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ $liquidationData['date']->format('d/m/Y') }}</p>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        {{ $liquidationData['date']->translatedFormat('l') }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">Comisión</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ $liquidationData['totals']['commission_percentage'] }}%</p>
                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ $liquidationData['appointments']->count() }}
                        pacientes atendidos</p>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <h3 class="text-lg font-semibold text-emerald-900 dark:text-emerald-100 mb-3">
            ...
            <div class="space-y-2">
                ...
                <div class="border-t-2 border-emerald-300 dark:border-emerald-700 pt-2 mt-2">
                    <div class="text-xs text-gray-600 dark:text-gray-400 mb-1.5 space-y-0.5">
                        <div class="flex justify-between">
                            <span>Comisión para el profesional ({{ $liquidationData['totals']['commission_percentage'] }}%
                                sobre pagos al centro):</span>
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span
                            class="text-xl font-bold {{ $liquidationData['totals']['net_professional_amount'] >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400' }}">
                    ...
                    <div class="bg-green-50 dark:bg-green-900/20 border-b border-green-200 dark:border-green-700 px-6 py-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <a href="{{ route('reports.professional-liquidation', ['professional_id' => $liquidationData['professional']->id, 'date' => $liquidationData['date']->format('Y-m-d'), 'print' => '1', 'liquidation_id' => $liquidation['id']]) }}"
                                    target="_blank"
                                    title="Imprimir solo esta liquidación"
                                    class="inline-flex items-center justify-center w-8 h-8 text-green-600 hover:text-green-800 hover:bg-green-100 dark:text-green-400 dark:hover:text-green-200 dark:hover:bg-green-800/40 rounded-lg transition-colors">
    ...
    @if (count($liquidationData['refunds']) > 0)
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
            <div class="bg-yellow-50 dark:bg-yellow-900/20 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    Montos devueltos que se descuentan de su liquidación:
                    ...
                    <table class="w-full">
                        ...
                        <tbody>
                            @foreach ($liquidationData['refunds'] as $refund)
                                <tr class="border-b border-gray-100 dark:border-gray-700">
                                    ...
                                    <td class="py-3 px-3">
                                        <div class="text-sm text-gray-900 dark:text-white">{{ $refund->description }}
                                        </div>
                                    </td>
                        ...
                        <tfoot>
                            <tr class="border-t-2 border-gray-300 dark:border-gray-600">
                                <td colspan="2"
                                    class="py-3 px-3 text-right font-semibold text-gray-900 dark:text-white">
                                    Total Reintegros:
                                </td>
    ...
    <!-- Footer -->
    <div
        class="text-center text-sm text-gray-500 dark:text-gray-400 mt-8 pt-6 border-t border-gray-200 dark:border-gray-600">
        Liquidación generada el {{ $liquidationData['generated_at']->format('d/m/Y H:i:s') }} por
        {{ $liquidationData['generated_by'] }}
    </div>
    </div>
