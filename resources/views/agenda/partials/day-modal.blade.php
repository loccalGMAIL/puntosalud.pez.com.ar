{{-- Panel de detalles del día (inline, sin overlay) --}}

{{-- Placeholder: sin día seleccionado --}}
<div x-show="!dayModalOpen"
     class="bg-white dark:bg-gray-800 rounded-lg shadow-lg
            flex flex-col items-center justify-center gap-3
            h-[calc(100vh-120px)] text-gray-400 dark:text-gray-500">
    ...
    <p class="text-sm">Seleccioná un día del calendario</p>
</div>

{{-- Panel de día --}}
<div x-show="dayModalOpen" x-cloak
     class="bg-white dark:bg-gray-800 rounded-lg shadow-lg flex flex-col
            h-[calc(100vh-120px)]">

    <!-- Header -->
    <div class="flex-shrink-0 bg-white dark:bg-gray-800 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between gap-3">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2 min-w-0">
            ...
            <div class="flex items-center gap-3 shrink-0">
                <button x-show="!isDayInPast()"
                        @click="openCreateModal(selectedDayDate, selectedProfessionalId)"
                        class="inline-flex items-center px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg shadow-sm transition-colors">
                    <svg class="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                    Nuevo Turno
                </button>
                <div x-show="isDayInPast()" class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
        ...
        <div x-show="daySchedule" class="mt-1.5 flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
    ...
    <div class="p-4 flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <div x-show="daySchedule" class="flex-1 min-h-0 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="relative bg-white dark:bg-gray-900" :style="`height: ${timelineHeightPx}px; min-height: 80px`">
                ...
                <!-- Fondo de filas alternadas por hora -->
                ...
                <template x-for="item in timelineLayout.items.filter(i => i.type === 'appointment')" :key="item.apt.id">
                    <div class="absolute left-[58px] right-2 rounded-sm transition-all z-10 select-none"
                         ...
                         @click="!isAppointmentInPast(item.apt) && openEditModal(item.apt)"
